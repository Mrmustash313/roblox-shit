...
local function CreateDecal()
	-- Create and configure the decal here
	local decal = Instance.new("Decal")
	decal.Texture = "rbxassetid://6196053948" -- Replace YOUR_DECAL_ID with the actual decal ID
...
function Wave:Update()
	for _, v in pairs(self._bones) do
		...
		local displacement = Gerstner(WorldPos, Settings.WaveLength, Direction, Settings.Steepness, Settings.Gravity, self._time)
		v.Transform = newCFrame(displacement)

		-- Check if the wave has reached its peak
		if displacement.Y >= 0 and random() <= 0.1 then
			-- Add a decal to simulate a wave crash
			local decal = CreateDecal()
			decal.Parent = v -- Attach the decal to the bone or part

			-- Remove the decal after a certain duration
			delay(3, function()
				decal:Destroy()
			end)
		end
